...
###write data to file
write.csv(summer[1:48,],file=paste("Summer",taxa,"DWA",".csv"), row.names=unique(years)[1:48])
write.csv(fall[1:48,],file=paste("Fall",taxa,"DWA",".csv"), row.names=unique(years)[1:48])
...
pdf(file=paste("taxa_",taxa,"_","DWA",".pdf",sep=""), width=8.5, height=11) #open graphics device for pdf

par(mfrow=(c(2,1)))
u=2
for (ssn in c("summer","fall")){
  u=u+1 #seasons numbered 1 to 4 starting with winter
  ...
  for (i in 1:length(spp)){
    ...
    if (length(kt)>1){dta[[i]]<-(rowMeans(get(ssn)[1:48,kt],na.rm=T))
                      mod[[i]]<-lm(rowMeans(get(ssn)[1:48,kt],na.rm=T)~as.numeric(unique(years)[1:48]), na.action=na.exclude)}
    
    if (length(kt)==1){dta[[i]]<-get(ssn)[1:48,kt]
                       mod[[i]]<-lm(get(ssn)[1:48,kt]~as.numeric(unique(years)[1:48]), na.action=na.exclude)}
  }
  ...
  legend("topright",pch=points, col=clrs,legend=paste(spp,"p=",round(p,4)), cex=1.5)
