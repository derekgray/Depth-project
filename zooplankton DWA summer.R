...
if(taxa=="Copepod"&stage=="copep"){Copepod.copep<-data.frame("DWA"=majspp.summer$all, "Year"=years, "Taxa"=rep("Copep", length(majspp.summer$all)))} #copepodites
if(taxa=="Copepod"&stage=="naup"){Copepod.naup<-data.frame("DWA"=majspp.summer$all, "Year"=years, "Taxa"=rep("Naup", length(majspp.summer$all)))} #nauplii
if(taxa=="Copepod"&stage=="adult"){Copepod.adult<-data.frame("DWA"=majspp.summer$all, "Year"=years, "Taxa"=rep("Adult", length(majspp.summer$all)))} #adult
if(taxa=="Rotifer"){Rotifer<-data.frame("DWA"=majspp.summer$all, "Year"=years, "Taxa"=rep("Rotifer", length(majspp.summer$all)))} #rotifers
if(taxa=="Cladoceran"){Cladoceran<-data.frame("DWA"=majspp.summer$all, "Year"=years, "Taxa"=rep("Cladoceran", length(majspp.summer$all)))} #cladoceran
...
groupkey<-data.frame(inkey=c("all", "Copepod","Cladoceran","Rotifer"), real=c("All Zooplankton", "Copepod","Cladoceran","Rotifer"))
currentgroup<-groupkey$real[which(groupkey$inkey==taxa)]
datas<-na.omit(data.frame("Date"=years,"DWA"=majspp.summer[[jj+1]],"Density"=majspp.density[[jj+1]]))
...
pl<-ggplot(datas, aes(x=Date, y=DWA, size=Density),legend=FALSE)+
  ...
  scale_y_reverse(name=paste(majspp2[[jj]],"Density-weighted Depth"))+
  geom_line(aes(x = Year, y = Fitted),size=1,data=results, colour="red") +
...
model1<-gls(DWA~Taxa/Year, correlation=corAR1(form=~1),na.action=na.exclude, data=allDWA) #temporal autocorrelation
...
results<-glht(model2, linfct = c("TaxaCopep:Year = 0","TaxaNaup:Year = 0",
                                 "TaxaAdult:Year = 0",
                                 "TaxaRotifer:Year = 0",
                                 "TaxaCladoceran:Year = 0"))
summary(results)

#Model fit
par(mar=c(4,5,2,2))
pdata <- expand.grid(Year=seq(1955, 2003, by=1), Taxa=c("Copep","Naup","Adult","Rotifer","Cladoceran"))
pdata$Temp <- allDWA$DWA
plot(pdata$Year,pdata$Temp, type="n",xlab="Year", ylim=c(0,200),ylab="Density-weighted Depth (m)", cex.lab=1.5,cex.axis=1.5)
points(pdata$Year[1:49], pdata$Temp[1:49], type="p", pch=15)
points(pdata$Year[50:98], pdata$Temp[50:98], type="p", pch=16, col="red")
points(pdata$Year[99:147], pdata$Temp[99:147], type="p", pch=17, col="green")
points(pdata$Year[148:196], pdata$Temp[148:196], type="p", pch=18, col="blue")
points(pdata$Year[197:245], pdata$Temp[197:245], type="p", pch=25, col="orange")
lines(fitted(model2)[1:49]~pdata$Year[1:49])
lines(fitted(model2)[50:98]~pdata$Year[50:98], col="red")
lines(fitted(model2)[99:147]~pdata$Year[99:147], col="green")
lines(fitted(model2)[148:196]~pdata$Year[148:196], col="blue")
lines(fitted(model2)[197:245]~pdata$Year[197:245], col="orange")
legend("topleft", c("Copepodites","Nauplii","Adult Copepods","Rotifers","Cladocerans"), pch=c(15, 16, 17,18,25),col=c("black","red","green","blue","yellow"), lty=1, lwd=2, cex=0.8)
...
pdata <- expand.grid(Year=seq(1955, 2003, by=1), Taxa=c("Copep","Naup","Adult","Rotifer","Cladoceran"))
pdata$Temp <- predict(model2, pdata, level=0)
plot(pdata$Year,pdata$Temp, type="n",xlab="Year", ylab="Temperature")
points(pdata$Year[1:49], pdata$Temp[1:49], type="b", pch=19, lwd=2)
points(pdata$Year[50:98], pdata$Temp[50:98], type="b", pch=22, lty=2, lwd=2)
points(pdata$Year[99:147], pdata$Temp[99:147], type="b", pch=23, lty=2, lwd=2)
points(pdata$Year[148:196], pdata$Temp[148:196], type="b", pch=24, lty=2, lwd=2)
points(pdata$Year[197:245], pdata$Temp[197:245], type="b", pch=25, lty=2, lwd=2)
legend("topleft", c("Copep","Naup","Adult","Rotifer","Cladoceran"), pch=c(19, 22, 23,24,25), lty=1, lwd=2)
