...
#This section pushes months foward by 1 so that quarters can align DJF MAM JJA SON
months<-as.numeric(substr(allsteps$Date,6,7))+1
months[which(months==13)]<-1
years<-as.numeric(substr(allsteps$Date,1,4))
...
allsteps$Date<-as.Date(paste(years,months,days,sep="-"))
dates<-as.Date(allsteps$Date)

overlap2<-xts(overlap, order.by=dates)
