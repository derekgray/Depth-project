...
#Check which phyto species was requested
if (phytspp!="all"){phyto2<-phyto2[which(phyto2$Species==phytspp),]} #which phyto group was requested?
...
ggplot(datas, aes(x=Date, y=DWA, size=Density),legend=FALSE)+
  ...
  scale_y_reverse(name=paste(phytspp,"Density-weighted Depth"))+
...
lines(fitted(model2)[126:150]~pdata$Year[126:150], col="purple")
lines(fitted(model2)[151:175]~pdata$Year[151:175], col="cyan")
legend("topleft", c("Diatom","Cyano","Green","Chrysophyte","Cryptophyte","Dinoflagellate","Picoplankton"), pch=c(15, 16, 17,18,25),col=c("black","red","green","blue","yellow"), lty=1, lwd=2, cex=0.8)
